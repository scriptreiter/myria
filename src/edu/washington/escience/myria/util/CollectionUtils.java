...
public final class CollectionUtils {
   ...
   * b2, c1 ...} ... {aN, bN, cN ...}]. In other words, the results are generated in same order as these nested loops:
   * 
   * <pre>
   * for (T a : [a1, a2 ...])
   *   for (T b : [b1, b2 ...])
   ...
   * <p>
   * Don't change the axes while iterating over their product, as a rule. Changes to an axis can affect the product or
   * cause iteration to fail (which is usually bad). To prevent this, you can pass clones of your axes to this method.
   * <p>
   * The implementation is lazy. This method iterates over the axes, and returns an Iterable that contains a reference
   * to each axis. Iterating over the product causes iteration over each axis. Methods of each axis are called as late
   * as practical. The Iterator constructs just one result at a time and doesn't retain references to them. For large
   * axes, this uses much less memory than a collection of the results.
   */
  public static <T> Iterable<T[]> cartesianProduct(final Class<T> resultType,
      final Iterable<? extends Iterable<? extends T>> axes) {
    return new Product<T>(resultType, newArray(Iterable.class, axes));
  }

  /**
   * Like {@link #cartesianProduct(Class,Iterable) cartesianProduct}(resultType, Arrays.asList(axes)), but slightly more
   * efficient.
   */
  @SafeVarargs
  public static <T> Iterable<T[]> cartesianProduct(final Class<T> resultType, final Iterable<? extends T>... axes) {
    return new Product<T>(resultType, axes.clone());
  }

  /**
   * Like {@link #cartesianProduct(Class,Iterable) cartesianProduct}(T.class, axes), except each result is a List
   * instead of an array. So, the result element type can be generic (unlike an array).
   */
  public static <T> Iterable<List<T>> cartesianProduct(final Iterable<? extends Iterable<? extends T>> axes) {
    return asLists(cartesianProduct(Object.class, axes));
    // The internal data structures are untyped, but the result is type safe.
  }

  /**
   * Like {@link #cartesianProduct(Iterable) cartesianProduct}(Arrays.asList(axes)), but slightly more efficient.
   */
  @SafeVarargs
  public static <T> Iterable<List<T>> cartesianProduct(final Iterable<? extends T>... axes) {
    return asLists(cartesianProduct(Object.class, axes));
  }

  // Don't make this public. It's really dangerous.
  private static <T> Iterable<List<T>> asLists(final Iterable<Object[]> arrays) {
  ...
  private static <T> T[] newArray(final Class<? super T> elementType, final Iterable<? extends T> from) {
  ...
  @SuppressWarnings("unchecked")
  private static <T> T[] newArray(final Class<? super T> elementType, final int length) {
  ...
  private static class Product<T> implements Iterable<T[]> {
    ...
    Product(final Class<T> resultType, final Iterable<? extends T>[] axes) {
    ...
    private static class ProductIterator<T> implements Iterator<T[]> {
      ...
      /**
       * The minimum index such that this.next() will return an array that contains _iterators[index].next(). There are
       * some special sentinel values: NEW means this is a freshly constructed iterator, DONE means all combinations
       * have been exhausted (so this.hasNext() == false) and _iterators.length means the value is unknown (to be
       * determined by this.hasNext).
       */
